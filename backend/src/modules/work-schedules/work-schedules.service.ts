import { Injectable, NotFoundException } from '@nestjs/common';
import { Op } from 'sequelize';
import { CreateWorkScheduleDto } from './dto/create-work-schedules.dto';
import { UpdateWorkScheduleDto } from './dto/update-work-schedules.dto';
import { FilterWorkSchedulesDto } from './dto/filter-work-schedules.dto';
import { WorkScheduleRepository } from './repositories/work-schedules.repository';

@Injectable()
export class WorkScheduleService {
  constructor(private readonly repository: WorkScheduleRepository) {}

  create(data: CreateWorkScheduleDto) {
    return this.repository.create(data);
  }

  async findAll(filters: FilterWorkSchedulesDto = {}) {
    const page = Math.max(Number(filters.page) || 1, 1);
    const limit = Math.min(Math.max(Number(filters.limit) || 10, 1), 100);

    const where: any = {};
    const professionalWhere: any = {};

    if (filters.professionalId) {
      where.professionalId = Number(filters.professionalId);
    }

    if (filters.barbershopId) {
      where.barbershopId = Number(filters.barbershopId);
    }

    if (filters.dayOfWeek) {
      where.dayOfWeek = Number(filters.dayOfWeek);
    }

    if (filters.startTime) {
      where.startTime = filters.startTime;
    }

    if (filters.endTime) {
      where.endTime = filters.endTime;
    }

    if (filters.isActive !== undefined) {
      where.isActive = String(filters.isActive) === 'true';
    }

    if (filters.professionalName) {
      professionalWhere.name = {
        [Op.iLike]: `%${filters.professionalName}%`,
      };
    }

    if (filters.professionalEmail) {
      professionalWhere.email = {
        [Op.iLike]: `%${filters.professionalEmail}%`,
      };
    }

    const include: any[] = [];

    include.push({
      association: 'professional',
      required: Object.keys(professionalWhere).length > 0,
      where:
        Object.keys(professionalWhere).length > 0
          ? professionalWhere
          : undefined,
    });

    include.push({
      association: 'barbershop',
      required: false,
    });

    const { rows, count } = await this.repository.findAndCountAll({
      where,
      include,
      limit,
      offset: (page - 1) * limit,
      order: [['id', 'ASC']],
    });

    return {
      data: rows,
      total: count,
      page,
      limit,
      lastPage: Math.ceil(count / limit),
    };
  }

  async findOne(id: number) {
    const item = await this.repository.findById(id);

    if (!item) {
      throw new NotFoundException('Horário de trabalho não encontrado');
    }

    return item;
  }

  async replace(id: number, data: CreateWorkScheduleDto) {
    const item = await this.findOne(id);
    await item.update(data);
    return item;
  }

  async update(id: number, data: UpdateWorkScheduleDto) {
    const item = await this.findOne(id);
    await item.update(data);
    return item;
  }

  async remove(id: number) {
    const item = await this.findOne(id);
    await item.destroy();

    return {
      message: 'Horário de trabalho removido com sucesso',
    };
  }
}