...
export class CategoryController {
  ...
  replace(@Param('id') id: string, @Body() dto: CreateCategoryDto) { return this.service.replace(Number(id), dto); }

  @Patch(':id')
  @Roles(UserRole.ADMIN, UserRole.BARBEIRO)
  @ApiOperation({ summary: 'Atualizar parcialmente Categoria' })
  update(@Param('id') id: string, @Body() dto: UpdateCategoryDto) { return this.service.update(Number(id), dto); }
